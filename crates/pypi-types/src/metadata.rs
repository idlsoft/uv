...
impl Metadata23 {
    ...
    pub fn parse_metadata_with_basedir(content: &[u8], base_dir: Option<&Path>) -> Result<Self, MetadataError> {
        ...
        let requires_dist = headers
            ...
            .map(|requires_dist| {
                LenientRequirement::from_str_and_workdir(&requires_dist, base_dir).map(Requirement::from)
            })
