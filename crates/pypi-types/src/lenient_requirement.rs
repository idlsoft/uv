...
impl<T: Pep508Url> FromStr for LenientRequirement<T> {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        Ok(Self(parse_with_fixups(Requirement::from_str, input, "requirement")?))
    }
}

impl<T: Pep508Url> LenientRequirement<T>  {
    pub fn from_str_and_workdir(input: &str, working_dir: Option<&Path>) -> Result<Self,  Pep508Error<T>> {
        Ok(Self(parse_with_fixups(|input| Requirement::parse_opt_workdir(input, working_dir), input, "requirement")?))
    }
}
...
impl FromStr for LenientVersionSpecifiers {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        Ok(Self(parse_with_fixups(VersionSpecifiers::from_str, input, "version specifier")?))
    }
}
