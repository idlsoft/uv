...
impl<'a, T: BuildContext> SourceDistributionBuilder<'a, T> {
    ...
    async fn build_distribution(
        ...
        let metadata = read_wheel_metadata(&filename, cache_shard.join(&disk_filename), Some(source_root))?;
...
fn read_wheel_metadata(
    ...
    Ok(Metadata23::parse_metadata_with_basedir(&dist_info, working_dir)?)
}
